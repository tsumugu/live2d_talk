...
export class LAppWavFileHandler {
  ...
  */
  public startFromArrayBuffer(aArrayBuffer: ArrayBuffer): void {
    // サンプル位参照位置を初期化
    this._sampleOffset = 0;
    this._userTimeSeconds = 0.0;
    ...
    if (!this.loadFromArrayBuffer(aArrayBuffer)) {
      return;
    }
  }
  ...
  public loadFromArrayBuffer(aArrayBuffer: ArrayBuffer): boolean {
    let ret = false;
    ...
    */
    ...
    const asyncWavFileManager = (() => {
      //this._byteReader._fileByte = await asyncFileLoad();
      this._byteReader._fileByte = aArrayBuffer;
